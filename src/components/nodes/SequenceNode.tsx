...
export default function SequenceNode({ id, data, selected }: SequenceNodeProps) {
  ...
  const isTargetCompatible = connectingHandle
    ? isHandleCompatible(connectingHandle, id, 'sequence', 'target')
    : false;
  const isSourceCompatible = connectingHandle
    ? isHandleCompatible(connectingHandle, id, 'sequence', 'source')
    : false;
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3.5 border-b border-zinc-900 pb-2">
        <div className="flex items-center gap-2 text-sky-400">
          <ListOrdered size={15} />
          <span className="text-[10px] font-bold uppercase tracking-wider">Sequence</span>
        </div>
